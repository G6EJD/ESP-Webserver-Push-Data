...
Adafruit_BME280 bme; // I2C bus connection
...
void handleHUMI() { // This function is called by the script to update the sensor value, in this example random data!
  String humi = String(bme.readHumidity(),2);
...
void handlePRES() { // This function is called by the script to update the sensor value, in this example random data!
  String pres = String(bme.readPressure() / 100.0F,2);
...
void setup(void) {
  Serial.begin(115200);
  Serial.println();
  Serial.println("SDA Pin is on : "+String(SDA));
  Serial.println("SCL Pin is on : "+String(SCL));
...
//  Wire.begin(4,5);
  ...
  if (!status) {
    ...
    Serial.println("Check your sensor address, the Adafruit library assumes 0x77");
    while (1);
  }
  //----------------------------------------------------------------
